...
const app = Vue.createApp({
    ...
    methods: {
        async doSearch() {
            ...
            const shouldRequestAgain = (() => {
                ...
                if(!!foundInFavorites){
                    ...
                    return (new Date().getTime() - new Date(lastRequest).getTime() > requestMaxTimeMs);
                }
                return false
            })(); //IIFE

            if(!!foundInFavorites && !shouldRequestAgain){
                console.log("Encontrado y estamos utilizando la version cacheada.");
        ...
        async doRequest(){
            try{
                console.log('No se encuentra o la version cacheada es muy vieja.');
